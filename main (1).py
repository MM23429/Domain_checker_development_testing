...
def get_subdomains_crtsh(domain, retries=3, delay=5):
    """
    Query crt.sh for certificate transparency logs to retrieve subdomains.
    Implements retries for 502/503 errors and timeouts.
    """
    url = f"https://crt.sh/?q=%25.{domain}&output=json"
    headers = {"User-Agent": "Mozilla/5.0 (compatible; SubdomainFinder/1.0)"}
    attempt = 0
                ...
                for entry in data:
                    ...
                    for sub in names.splitlines():
                        if sub.endswith(domain):
                            subdomains.add(sub.lstrip("*.").strip())
                return list(subdomains)
            elif response.status_code in [502, 503]:
                st.info(f"Received {response.status_code} from crt.sh, retrying in {delay} seconds... (Attempt {attempt+1}/{retries})")
                time.sleep(delay)
                attempt += 1
            else:
                st.error(f"Error: Received status code {response.status_code} from crt.sh")
                return []
        except requests.exceptions.Timeout as e:
            st.error(f"Timeout in get_subdomains_crtsh: {e} - Retrying in {delay} seconds (Attempt {attempt+1}/{retries})")
            time.sleep(delay)
            attempt += 1
        except requests.exceptions.RequestException as e:
            st.error(f"RequestException in get_subdomains_crtsh: {e} - Retrying in {delay} seconds (Attempt {attempt+1}/{retries})")
            time.sleep(delay)
            attempt += 1
    st.error("Exceeded retry limit for crt.sh.")
    return []
...
def get_subdomains_certspotter(domain):
    ...
    try:
        ...
        if response.status_code != 200:
            st.error(f"CertSpotter API error: {response.status_code} {response.text}")
            return []
        data = response.json()
        subdomains = set()
        for entry in data:
        ...
        st.error(f"Exception in get_subdomains_certspotter: {e}")
        return []

def get_subdomains_securitytrails(domain, api_key):
    """
    Query SecurityTrails API to retrieve subdomains.
    Requires a valid API key.
    """
    headers = {"APIKEY": api_key, "User-Agent": "Mozilla/5.0 (compatible; SubdomainFinder/1.0)"}
    url = f"https://api.securitytrails.com/v1/domain/{domain}/subdomains"
    try:
        response = requests.get(url, headers=headers, timeout=REQUEST_TIMEOUT)
        if response.status_code != 200:
            st.error(f"SecurityTrails API error: {response.status_code} {response.text}")
            return []
        data = response.json()
        subs = data.get("subdomains", [])
        return [f"{sub}.{domain}" for sub in subs]
    except Exception as e:
        st.error(f"Exception in get_subdomains_securitytrails: {e}")
        return []

def bruteforce_subdomains(domain, prefixes):
    """
    Brute force subdomains using a provided list of wordlist prefixes.
    """
    return [f"{prefix}.{domain}" for prefix in prefixes]

async def verify_subdomain(session, url):
    """
    Verify if a subdomain is active by sending an HTTP request.
    """
    try:
...
async def verify_subdomains_async(subdomains):
    """
    Asynchronously verify a list of subdomains.
    """
    verified = []
    async with aiohttp.ClientSession() as session:
        ...
        for sub in subdomains:
            for scheme in ["http", "https"]:
                url = f"{scheme}://{sub}"
                tasks.append(asyncio.create_task(verify_subdomain(session, url)))
        results = await asyncio.gather(*tasks)
        for res in results:
            if res:
                parsed = urlparse(res)
                verified.append(parsed.netloc)
    return list(set(verified))

def aggregate_subdomains(domain, api_key=None, wordlist=None):
    """
    Aggregate subdomains from multiple methods.
    """
    subdomains = set()
    st.write("[*] Querying crt.sh ...")
    crt_subs = get_subdomains_crtsh(domain)
    if crt_subs:
        subdomains.update(crt_subs)
        st.write(f"[+] Found {len(crt_subs)} subdomains from crt.sh")
    else:
        st.write("[!] crt.sh returned no results, trying CertSpotter as fallback...")
        cert_subs = get_subdomains_certspotter(domain)
        subdomains.update(cert_subs)
        st.write(f"[+] Found {len(cert_subs)} subdomains from CertSpotter")
    if api_key:
        st.write("[*] Querying SecurityTrails ...")
        st_subs = get_subdomains_securitytrails(domain, api_key)
        subdomains.update(st_subs)
        st.write(f"[+] Found {len(st_subs)} subdomains from SecurityTrails")
    if wordlist:
        st.write("[*] Running brute-force enumeration ...")
        bf_subs = bruteforce_subdomains(domain, wordlist)
        subdomains.update(bf_subs)
        st.write(f"[+] Generated {len(bf_subs)} potential subdomains from brute force wordlist")
    return list(subdomains)

# -------------------- Original Domain Checker Functions --------------------
http_headers = {
    "User-Agent": (
...
# Create tabs including the new "Subdomain Finder" tab.
tabs = st.tabs(["HTTP Check", "DNS Lookup", "WHOIS Check", "TLS/SSL Certificate Check", "Advanced Check", "Subdomain Finder"])
...
with tabs[5]:
    ...
    st.markdown(
        """
        This tool aggregates subdomains for a given target domain using multiple sources:
        - **crt.sh** (certificate transparency logs)
        - **CertSpotter** as a fallback
        - **SecurityTrails** (if a valid API key is provided)
        - An optional brute-force wordlist approach

        It then verifies the discovered subdomains asynchronously.
        """
    )
